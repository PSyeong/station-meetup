...
EXTRA_EDGES = [
    ("병점역", "서동탄역", "경부선"),   # 병점역에서 갈라지는 서동탄지선
    ("금천구청역", "광명역", "경부선"),  # 금천구청역에서 갈라지는 광명역 셔틀 지선
    ("구로역", "구일역", "경인선"),     # 구로역에서 갈라지는 경인선 분기 시작점
    ...
    ("신답", "용두(동대문구청)", "2호선"),
    ("용두(동대문구청)", "신설동", "2호선"),
    ("이촌역", "용산역", "경원선"),     # 경원선 완행 구간이 실제로 이어지는 지점
]

# 1호선은 경원선/1호선/경부선/경인선 4개 원본 구간을 이어붙여 만든 것이라, 구간이
# 바뀌는 경계에서 실제로는 인접하지 않은 역끼리 연결될 수 있다.
# - 경부선(남영~천안) 끝에서 경인선(구일~인천) 시작으로 넘어가는 지점: 둘 다
#   구로역에서 갈라지는 별개 지선이라 순서상 이어붙이면 안 된다(위 EXTRA_EDGES의
#   구로-구일 연결로 대신한다).
# - 경원선(소요산~이촌) 끝에서 1호선(청량리~서울역) 시작으로 넘어가는 지점: 이촌역은
#   실제로 용산역과 이어지지 순서상 다음인 청량리역과 이어지지 않는다(위
#   EXTRA_EDGES의 이촌역-용산역 연결로 대신한다).
# (담당1 수동 검증, 2026-08-15)
