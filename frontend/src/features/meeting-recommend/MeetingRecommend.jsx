...
import { generateMockRecommendations } from "./mockRecommendation.js"
import { generateMockPlaces } from "./mockPlaces.js"
...
export default function MeetingRecommend() {
  ...
  function handleFindMeetingPoint(origins, mode) {
    const results = generateMockRecommendations({
      graph,
      origins,
      mode,
      topK: 3,
  ...
  function handleShowPlaces() {
    setPlacesLoading(true)
    setPlaces(null)
    setScene("places")
    // mock: 실제 API 연동 시 이 부분을 recommend_places_for_selection() 호출로 교체
    setTimeout(() => {
      const result = generateMockPlaces({
        meetingStations: recommendations,
        selectedIndex,
      })
  ...
  return <InputScene graph={graph} onSubmit={handleFindMeetingPoint} />
}
